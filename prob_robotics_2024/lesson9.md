...
- やること: 移動時のベイズフィルタの式をガウス分布だけの演算に
    - 次の2つの問題を解く
        - 問題1: $\V{x}_{t-1}$が分かっていると仮定したとき、$\V{u}_t$がばらつくと$\V{x}_t$はどんなガウス分布になるだろうか？
            - $p(\V{x} | \V{x}_{t-1}, \V{u}_t)$の近似が必要
        - 問題2: $\V{x}_{t-1}$が$b_{t-1}$にもとづいてばらついているとき、$b_{t-1}$から$\hat{b}_t$をどう計算するか
            - いろんな場所の$\V{x}_{t-1}$を写像する計算に近似が必要
...
- 行列とベクトルの計算式をシミュレータに実装
    - 同時刻に複数のランドマークを観測した場合:
    前ページの処理を繰り返す
    - センサ値のバイアスで誤差楕円から出てしまうがこれはパーティクルフィルタと同じ
    （詳解4章の独立同分布の問題がある）

![bg right:30% 100%](./figs/kalman_filter.gif)
