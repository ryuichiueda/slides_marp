...
- 利点: <span style="color:red">計算量が小さい</span>
    - 平均値と共分散行列の演算だけでベイズフィルタを実装可能
    - 状態の次元が高くても適用可能<br />　
- 問題1: ガウス分布なので次のような状況で信念が表現しずらい
...
- ベイズフィルタの式
    - $\hat{b}_t(\V{x}) = \big\langle p(\V{x} | \V{x}', \V{u}_t) \big\rangle_{b_{t-1}(\V{x}')} $<span style="color:red">$= \int_{\V{x}' \in \mathcal{X}} p(\V{x} | \V{x}', \V{u}_t) b_{t-1}(\V{x}') d\V{x}'$</span><br />　
...
- $A_t = \dfrac{\partial \V{f}}{\partial \V{u}}\Big|_{\V{x}=\V{x}_{t-1},\V{u}=\V{u}_t}$
    - 近似した状態方程式（再掲）: $\V{x}_t \approx \V{f}(\V{x}_{t-1}, \V{u}_t) + A_t (\V{u}_t' - \V{u}_t)$<br />　
- 解釈
...
- 次にやること
    - $\V{x}_t \approx \V{f}(\V{x}_{t-1}, \V{u}_t) + A_t (\V{u}_t' - \V{u}_t)$について、<br />$\V{u}'_t$がばらつくときの$\V{x}_t$の分布を求める<br />　
- 分布の式
...
2. 共分散行列の定義から
    - $R_t = \left\langle  (\V{x}_t - \V{\mu}_t) (\V{x}_t - \V{\mu}_t)^\top \right\rangle_{\mathcal{N}(\V{u}, M_t)}$<br />
	...
	<span style="color:red">$ = A_t M_t A_t^\top$</span><br />
...
- 代入するガウス分布
   ...
   <span style="color:red">$= \eta \exp\left\{ -\frac{1}{2} \left[\V{x} - \V{f}(\V{x}',\V{u}_t) \right]^\top R_t^{-1} \left[ \V{x} - \V{f}(\V{x}',\V{u}_t) \right] \right\}$</span>
    ...
    <span style="color:red">$= \eta \exp\left\{-\frac{1}{2}(\V{x}' - \V{\mu}_{t-1})^\top \Sigma_{t-1}^{-1} (\V{x}' - \V{\mu}_{t-1})  \right\}$</span><br />　
- 代入して式を整理
...
<span style="font-size:70%">$= \\begin{pmatrix} 1 & 0 & \\nu_t\\omega_t^{-1}\{\\cos(\\theta_{t-1} + \\omega_t\\Delta t) - \\cos \\theta_{t-1} \} \\\\ 0 & 1 & \\nu_t\\omega_t^{-1}\{\\sin(\\theta_{t-1} + \\omega\\Delta t) - \\sin \\theta_{t-1} \} \\\\ 0 & 0 & 1 \\end{pmatrix} \\Bigg|_{ \\boldsymbol{x}_{t-1} = \\boldsymbol{\\mu}_{t-1}}$</span><br />　
...
- 指数部に近似式を代入
    - <span style="font-size:90%">$ = - \dfrac{1}{2}\left[ \V{x} - \V{f}(\V{\mu}_{t-1}, \V{u}_t) - F_t(\V{x}' - \V{\mu}_{t-1}) \right]^\top R_t^{-1} \left[ \text{省略} \right] \\\\ - \dfrac{1}{2}( \V{x}' - \V{\mu}_{t-1} )^\top \Sigma_{t-1}^{-1} ( \text{省略} ) $</span><br />
<span style="font-size:90%">$ = - \dfrac{1}{2}\left[ \V{x} - \V{f}(\V{\mu}_{t-1}, \V{u}_t) \right]^\top (F_t \Sigma_{t-1} F_t^\top + R_t)^{-1} \left[ \V{x} - \V{f}(\V{\mu}_{t-1}, \V{u}_t) \right] $</span><br />
...
- $ \hat{b}_t(\V{x}) = \eta\int_{\V{x}' \in \mathcal{X}} \exp\big\{$指数部$ \big\}  d\V{x}'$<br />
...
$ = \eta \exp\{L(\V{x}) \}$<br />
$ = \eta e^{-\frac{1}{2}\left[ \V{x} - \V{f}(\V{\mu}_{t-1}, \V{u}_t) \right]^\top (F_t \Sigma_{t-1} F_t^\top + R_t)^{-1} \left[ \V{x} - \V{f}(\V{\mu}_{t-1}, \V{u}_t) \right] }$<br />
...
- 困るのかどうか$\Longrightarrow$困らない
    ...
    - 物理的な理由: 速度・角速度の雑音がなくても不確かさ$F_t \Sigma_{t-1} F_t^\top$は存在<br />
（本来は数式で証明しなければならない）
