...
- 次の「裏」が出たとき
    ...
    $\qquad\qquad= (1/2 \cdot 3/5) / (1/2 \cdot 3/5 + 2/3 \cdot 2/5) = 17/100$
    - $P(B | 裏) = 1 - 17/100 = 83/100$
- 答え: いまのところコインA、Bである確率はそれぞれ17%、83%
...
- $\V{x}_3$の分布を次のように周辺化して求める
    - $p(\V{x}_3) = \langle p(\V{x}_3| \V{x}_1)\rangle_{p(\V{x}_1)}$
