...
- 観測分布の値$p_j (\V{z}_j | \V{x})$の比で数値化可能
    - 例: $p_j (\V{z}_j | \V{x}^{(i)}) = 0.02, p_j (\V{z}_j | \V{x}^{(k)}) = 0.01$なら$\V{x}^{(i)}$の方が$\V{x}^{(k)}$より2倍<span style="color:red">尤もらしい</span>
...
- 尤度関数（観測モデル）をどう決めるか
    - シミュレータの実装を見ると分かるが実際のセンサはブラックボックス
    - 状態遷移モデル同様、実験で
...
<span style="color:red">実験で得られるモデルは、実験条件の枠内限定。自律ロボットは枠を超えることを期待される。そこが難しい。<span style="font-size:80%">（人間も然り）</span></span>
