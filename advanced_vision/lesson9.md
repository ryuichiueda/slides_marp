...
- 同じタスクでもロボットの動きが何通りもある（いくつもモードがある）ので、それらが混ざらないようにする
    - 潜在空間中のガウス分布に様々なモードを配置
    - デコーダに$\boldsymbol{z} = \boldsymbol{0}$を指定$\rightarrow$潜在空間中の真ん中の動作シーケンスが得られる
        - さらに画像やセンサ値で条件付けると、タスクに対して適切な動作に
- CVAEのエンコーダは$\boldsymbol{z}$の分布（共分散ゼロのガウス分布）を学習
