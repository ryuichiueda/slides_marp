...
- 「良くない」と社会的に思われている行動も、個人レベルではそのほうがいいという決定の上で行動していると解釈できる
    - 社会的な軋轢の回避、個人的な苦痛の除去や快楽の追及、惰性で報酬がよくなる
...
- こういう式を使うこともある（今回は取り上げません）
    * $\sum_{i=1}^T \gamma^i \ell_i + \gamma^T V(\boldsymbol{x}_T)$（$0 < \gamma < 1$）
        - 未来に観測される報酬や価値を低く見積もる（=今がよければいいや）
