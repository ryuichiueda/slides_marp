...
- ネットワーク全体は$y=f(x|w)$という形（wには\thetaも入る）
    - 出力の誤差$\Delta y$に対してだけ$\Delta w$パラメータを変える
    - $\Delta y=\round y/\round w \Delta w$（実際は少し割り引く）
...
- こうやると解ける
    ...
    - 誤差の大きさは、そのニューロンの影響を考えて加減する
    - 送られてきた誤差が小さくなるようにパラメータを変える
