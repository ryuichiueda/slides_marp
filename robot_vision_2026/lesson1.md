...
- 基礎をおさえて（効率悪いので）いちいち新しいものを試して驚き屋をしなくても「そういうことね」と言えるくらいになりましょう
...
- 補足
    - 期末テストは選択式にします
        - 数学の問題は出しません。免許の試験のような問題
    - 宿題無いので製作論がんばって
    - 連絡はmanabaで。Slackは使いません。
...
- 右図のような構造に簡略化（人工ニューロン）
    ...
    - <span style="color:red">ひとつの値だけ信号$y$を出す</span>
        - $w_1 x_1 + w_2 x_2 + \dots + w_n x_n \ge b$
        なら$1$
            - $b$: 閾値（バイアスとも）
        - 接続先は複数だが同じ値を送信
