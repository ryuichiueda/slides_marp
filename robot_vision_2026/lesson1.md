...
- 機械学習の理論的背景
- 方法、原理が単純
    - $\boldsymbol{y} = \boldsymbol{f}(\boldsymbol{x}|\boldsymbol{w})$に対し$\arg\!\min_{\boldsymbol{w}} \mathcal{L}(\boldsymbol{w} |\boldsymbol{d})$を解くだけ（大変だけど）
        - $\mathcal{L}$: <span style="color:red">損失関数</span>
            - $\boldsymbol{y}$が欲しい出力からどれだけずれているか数値化する関数
        - $\boldsymbol{w} = \{w_1, w_2, \dots, w_N\}$: パラメータ
        - $\boldsymbol{d} = \{ d_1, d_2, \dots, d_M\}$: データ
    - パラメータ
