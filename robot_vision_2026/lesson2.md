...
- $\boldsymbol{w}$のうち、ある層$m$にあるパラメータ$w$を動かしたい
    ...
    $= \dfrac{\partial \boldsymbol{f}(\boldsymbol{x})}{\partial w}\Big|_{\boldsymbol{x}'}\cdot (\boldsymbol{f}(\boldsymbol{x}') - \boldsymbol{y}')$ 
        - $J_{\boldsymbol{f}}(\boldsymbol{x}') = \dfrac{\partial f_i(\boldsymbol{x})}{\partial w}\Big|_{\boldsymbol{x} = \boldsymbol{x}'}$
    $= \dfrac{\partial \boldsymbol{f}(\boldsymbol{x}')}{\partial w}\cdot \boldsymbol{e}'$（$\boldsymbol{e}'$: 誤差のベクトル）
