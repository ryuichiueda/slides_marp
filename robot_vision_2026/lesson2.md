...
- $\boldsymbol{w}$のうち、ある層$m$にあるパラメータ$w$を動かしたい
    - $\dfrac{\partial}{\partial w}\mathcal{L}(\boldsymbol{w} , \boldsymbol{x} | \boldsymbol{y}) = \sum_{i=1}^k \dfrac{\partial f_i(\boldsymbol{x})}{\partial w}\{ f_i(\boldsymbol{x}) - y_i \}$（$\leftarrow$内積）
