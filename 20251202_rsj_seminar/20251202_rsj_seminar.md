...
### 危険な箇所に近づいたら回避するルールを加えるのではだめ？

- OK。ただし
    - 作った$\boldsymbol{\pi}_\text{wall}$と計画結果$\boldsymbol{\pi}_\text{plan}$が矛盾しないように（難しい）
        - 矛盾がない: 両方の方策から計算される$V^\boldsymbol{\pi}$に停留点ができないように
            - $V^\boldsymbol{\pi}$あるいはその微分を計算しないと評価できない
- よくある矛盾: 角曲がりたい↔角の壁から遠ざかりたいで無限ルーブに
- ダメなやつ: 同じ状態に対して別の方策を混在
    - 航空機事故によくあるやつ（名古屋空港での中華航空機の事故）
