...
- 「厳密に解く」: <span style="color:red">危ない状態はゴール</span>
    - $V(\boldsymbol{x}_\text{f})$を大きな値で固定
        ...
        - ゴールに行く・危険回避を統一的に解ける
- 探索や制御、最適化では「境界（制約）条件」として扱ってしまう（いいの？）
...
- 失: 一般化されすぎていて既存の制御理論から離れるか、再解釈が必要になる
    - リアプノフやポントリャーギンの話、あらゆる微分方程式の話
        - <span style="font-size:90%">前提にこれのある人とない人では話が合わない</span>
    - 再解釈の例: グラフ上で微分を定義してなにかするなど
        - 似た例: 学部のときの指導教官の故湯浅先生の研究
...
- <span style="font-size:70%">ちなみに私の博士論文のテーマも「方策の圧縮」です。確率ロボティクスではないです。</span>
...
- 制御という観点から見ると、「方策寄り」か「価値寄り」かという軸で話ができる（大雑把ですが）
    - 模倣学習寄りの手法: 方策を記録
        - VAEや拡散モデル、FMでさまざまな動作シーケンスを学習・生成
    - その他汎用的な手法: 価値を推定
        - 指示と画像から次の行動だけを出力
            - 状態の価値がよくなるようにひとつずつ行動を選択
...
- もし、空間中のさまざまな場を$O(1)$で正確に計算できる計算機ができたらどうなる？
    - 人工ニューラルネットでいうと、各パラメータが磁場のように一瞬で決まる計算機
    - 粘菌のコンピュータはこれに近い
