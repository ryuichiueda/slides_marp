...
- 前ページの現象を説明する確率分布
- Pr{コインを$n$枚投げたときに表が$m$枚出る}
    - 前ページの「誤差が正、負になる」がコインの裏表に相当
    - とりあえず表の出る確率を$1/2$として計算してみましょう
        - 加法定理を使います
        - わからん人はとりあえず「2枚投げたら表が$m$枚（$m = 0, 1, 2$）」
        をどうぞ
- 答えは次のページ
...
- 表が$m$回出るときの具体的な並びに事象を分解して加法定理を適用
    - Pr{コインを$n$枚投げたときに表が$m$枚出る}
= Pr{表表・・・（$m$回）裏裏・・・（$n-m$回）}
$+ ...$
$+$Pr{裏裏・・・（$n-m$回）表表・・・（$m$回）}
= $(1/2)^{n} + ... + (1/2)^{n}$
（$\because$具体的な並びが出る確率: $1/2$の確率ですべて表裏の予想が当たる）
- 具体的な並び方: $_nC_m = \dfrac{n!}{(n-m)!m!}$種類
- したがって Pr{コインを$n$枚投げたときに表が$m$枚出る} = $_nC_m(1/2)^n$
    - $n=2, m=0,1,2$で計算してみましょう
- 表が出る確率が$p$のときは？
...
- 並び方の種類は$_nC_m$のまま
- したがって Pr{コインを$n$枚投げたときに表が$m$枚出る} = $_nC_m p^m(1-p)^{n-m}$

---

### 二項分布の式

- <span style="color:red">$P(m | n, p) =_n\!\!C_m p^m (1-p)^{n-m}$</span>
    - 分布全体は$B(n, p)$と表す
- $B(n, p)$を絵に描いてみよう
    - $n$、$p$は適当にえらんでください（簡単なものを）
    - 横軸に$m = 0, 1, 2, \dots, n$、縦軸に$P(m | n, p)$の値
- コツ: どんな形か調べたいだけなら、$_nC_m$を計算する必要はない
    - 比例定数なので
...
- ついでに
    - その1: 分布$P(x)$にしたがう$x$の期待値の表記法
        - $\langle x \rangle_P$と書いたり$E_P( x )$と書いたり
            - この資料では前者を使います
    - その2: 右のようにある範囲でどこも確率が同じ分布を<span style="color:red">一様分布</span>と呼びます
