...
- a$_4$を除いたa$_{1:3}$が1つ以上起動する確率の計算
   - 「1つ以上起動」を排反な事象に切り分ける
       - a$_1$が起動: 確率$0.8$
       - a$_1$が起動せず，a$_{2}$が起動: 確率: $0.2\cdot 0.8 = 0.16$
       - a$_{1:2}$が両方起動せず，a$_{3}$が起動: 確率$0.2^2 \cdot 0.8 = 0.032$
...
- a$_4$が起動しないと冗長化したA$_1$は起動しない
    - （冗長化を意識したシステムでは）<span style="color:red">単一故障点（単一障害点）</span>と呼ばれる
- a$_{1:3}$をひとかたまりのシステムとすると、a$_{1:3}$とa$_4$は直列
- 冗長化したA$_1$の計算結果
    - a$_4$の起動率が$0.95$: $0.992\cdot 0.95 = 0.94$（もとのA$_1$の起動率を上回る）
    - a$_4$の起動率が$0.8$: $0.992\cdot 0.8 = 0.79$（もとのA$_1$の起動率を下回る）
- 結局、a$_4$の信頼性が相当高くないと冗長化の効果は少ない
...
- 余事象を使用
    - 「a$_{1:3}$のどれかが起動」の余事象は「a$_{1:3}$が全て起動しない」
