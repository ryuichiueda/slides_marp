...
- 加法定理の逆をやる
    - $\Pr\{$ロボットが起動$\} =\sum_{Z = \text{X}, \text{Y}} \Pr\{$ロボットが起動$\cap Z\}$
        - 「電源回路がX君作」と「電源回路がY君作」は排反で、
        合わせると全事象になるので成立
- 乗法定理を適用
    - $\cdots =\sum_{Z = \text{X}, \text{Y}} \Pr\{$ロボットが起動$| Z\}\Pr\{Z\}$
...
- もともと考えていなかった条件をあとから入れることができる
    ...
    $=\Pr\{$ロボットが起動$|$X$\}\Pr\{$X$\} + \Pr\{$ロボットが起動$|$Y$\}\Pr\{$Y$\}$
        ...
        - 条件が書いてなくても、実はあらゆることが条件になっている
            - $\Pr\{$ロボットが起動$\}=\Pr\{$ロボットが起動$|$世の中のあらゆること$\}$
        - ロボットの起動と独立な事象を除去すると・・・
            $\cdots = \Pr\{$ロボットが起動$|$ロボットの起動に関係するあらゆること$\}$
