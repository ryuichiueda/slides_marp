...
- $p_{i-1}(t|x_i)= \dfrac{p_{i-1}(x_i|t)}{p_{i-1}(x_i)}\cdot p_{i-1}(t)$の各要素を見ていきましょう
    ...
    $=\int_{t=0}^1 t p_{i-1}(t)\text{d}t=$<span style="color:red">$\langle  \rangle_{p_{i-1}(t)}$</span>
    - $x_i$の期待値=事前分布での完走率の期待値

---

- したがって
    - $p_{i-1}(t|x_i)= \dfrac{t^{x_i}(1-t)^{1-x_i}}{\langle t' \rangle_{p_{i-1}(t')}}\cdot p_{i-1}(t) = \eta t^{x_i}(1-t)^{1-x_i} p_{i-1}(t)$
        - 補足: $t'$の「$'$」は、他の$t$と違う扱いをするという意味
